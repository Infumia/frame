...
public final class ServiceRegisterViews
    implements PipelineService<PipelineContextFrame.RegisterViews, Collection<View>> {

    public static final PipelineService<
        PipelineContextFrame.RegisterViews,
        Collection<View>
    > INSTANCE = new ServiceRegisterViews();
    ...
    public CompletableFuture<Collection<View>> handle(
        @NotNull final PipelineContextFrame.RegisterViews ctx
    ) {
    ...
    @Override
    public String key() {
        return ServiceRegisterViews.KEY;
    }

    private ServiceRegisterViews() {}
}
